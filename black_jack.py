...
def take_card (hand, img_list):
    ...
    cards.remove(hand[-1])
    img_list = img_list.append(cards_imgs[cards_stable.index(hand[-1])])
...
def add_cards(player, computer):
        ...
        if 11 in player and (calc_score(player) > 21 or calc_score(player[-2:]) > 21):
            i = player.index(11)
            player[i] = 1
...
def play():    
    ...
    taking = True
    while taking:
        player, taking, winning = add_cards(player, computer)
    ...
    if calc_score(player) < 21:
        while i > 0:
            ...
            if calc_score(computer) < 16:
                take_card(computer, img_computer)
            elif calc_score(computer) >= 16 and calc_score(computer) <= 19:
                to_take_card = random.choice(true_false)
                if to_take_card:
                    take_card(computer, img_computer)
            else:
                i -= 2  
            if 11 in computer and (calc_score(computer) > 21 or calc_score(computer[-2:]) > 21):
                i = computer.index(11)
                computer[i] = 1
...
while True:
    ...
    while decision != 'y' and decision != 'n':
    	decision = input("Inappropriate symbol. Type 'y' to get another card, type 'n' to pass: ")
    if decision == 'y':
